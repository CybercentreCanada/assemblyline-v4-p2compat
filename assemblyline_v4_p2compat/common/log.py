...
from assemblyline_v4_p2compat.common.logformat import AL_LOG_FORMAT, AL_SYSLOG_FORMAT, AL_JSON_FORMAT
    ...
    if config.logging.log_to_file:
        ...
        if log_level <= logging.DEBUG:
            dbg_file_handler = logging.handlers.RotatingFileHandler(
                os.path.join(config.logging.log_directory, '{name}.dbg'.format(name=name)),
        ...
        if log_level <= logging.INFO:
            op_file_handler = logging.handlers.RotatingFileHandler(
                os.path.join(config.logging.log_directory, '{name}.log'.format(name=name)),
                maxBytes=10485760, backupCount=5)
        ...
        if log_level <= logging.ERROR:
            err_file_handler = logging.handlers.RotatingFileHandler(
                os.path.join(config.logging.log_directory, '{name}.err'.format(name=name)),
                maxBytes=10485760, backupCount=5)
